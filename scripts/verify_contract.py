...
def main() -> int:
    ...
    require(
        "MOONLI_ENV: production" in ci_workflow
        and "/internal/test/pipeline" in ci_workflow
        and "MOONLI_ENV=production" in release_workflow
        and "/internal/test/pipeline" in release_workflow,
        "CI and release must smoke-test Test Calls in production mode",
        findings,
    )
    require(
    ...
    require("location = /internal/routing" in nginx, "routing settings route is not exposed", findings)
    require(
        "location /internal/test/" in nginx,
        "authenticated test-call routes are not exposed through Nginx",
        findings,
    )
    ...
    require(
        "location /internal/production/pipelines/" in nginx,
        "pipeline-specific production settings routes are not exposed",
        findings,
    )
    ...
    require('id="routingVlessUri" type="password"' in html, "private VLESS routing UI is missing", findings)
    require(
        "ghcr.io/xtls/xray-core:26.7.11@sha256:" in xray_dockerfile
        and "vless-proxy:" in compose
        and "/run/moonli-secrets:ro" in compose,
        "pinned private Xray sidecar is missing",
        findings,
    )
