...
rm -rf "$output_path"
mkdir -p "$output_path" "$stage/deploy/nginx" "$stage/deploy/xray" "$stage/updater/systemd"
...
install -m 0644 "$root/deploy/xray/Dockerfile" "$stage/deploy/xray/Dockerfile"
install -m 0644 "$root/deploy/xray/direct.json" "$stage/deploy/xray/direct.json"
